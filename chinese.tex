...
\centerline{\heiti\bf\zihao{2} 四聚物、三聚物在栅格上的计数表示} \vskip .1in
...
文章Kasteleyn$^{[1]}$中，非常巧妙的将二聚物覆盖问题转化成了一个矩阵行列式的问题，最终得到了覆盖数的计算公式，非常漂亮的结构。既然二聚物覆盖的计算公式已经得到，那么三聚物，四聚物覆盖呢？这是一个很有趣的问题。本文沿着作者在思考这个问题时的思维顺序进行写作，中间有很多错误的思路。首先会讨论四聚物覆盖数的计算公式，最后会讨论三聚物的覆盖。并且对pfaffian 和matrix的理论意义进行适当的探讨。
...
\centerline{\Large\bf  The statistics of trimers and tetramers on a lattice }\vskip
.1in
